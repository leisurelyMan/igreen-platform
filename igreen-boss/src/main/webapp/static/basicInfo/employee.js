...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../basicInfo/employeeList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>人员姓名</b>','<b>职务</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'name',index:'name', width:120,sortable:false},
					{name:'job',index:'job', width:200,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../basicInfo/addEmployee.do',//这里是接收数据的URL
						data:$('#employeeform').serialize(),//传给后台的数据，多个参数用&连接
	...
	$("#search").click(function(){
		...
		$("#list2").jqGrid("setGridParam",{postData:{name:searchName,job:searchJob},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
...
function edit(employeeId){
	...
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../basicInfo/getOneEmployee.do',//这里是接收数据的URL
		data:'employeeId='+employeeId,//传给后台的数据，多个参数用&连接
