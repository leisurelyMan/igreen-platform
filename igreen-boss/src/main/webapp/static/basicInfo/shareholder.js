...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../basicInfo/shareholderList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>股东名称</b>','<b>认缴出资额(万元)</b>','<b>币种</b>','<b>出资比例</b>','<b>出资日期</b>','<b>国籍</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'shareholderName',index:'shareholderName', width:80,sortable:false},
					{name:'capital',index:'capital', width:80,sortable:false},
					{name:'currency',index:'currency', width:80,sortable:false}, 
					{name:'ratio',index:'ratio', width:80,sortable:false},
					{name:'contributiveTime',index:'contributiveTime', width:80,sortable:false},
					{name:'nationality',index:'nationality', width:80,sortable:false},
