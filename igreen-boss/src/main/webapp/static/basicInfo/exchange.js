...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../basicInfo/exchangeList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>变更事项</b>','<b>变更前内容</b>','<b>变更后内容</b>','<b>变更日期</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'exchangeItem',index:'exchangeItem', width:100,sortable:false},
					{name:'beforeItem',index:'beforeItem', width:150,sortable:false},
		 		    {name:'afterItem',index:'afterItem', width:150,sortable:false},
					{name:'exchangeTime',index:'exchangeTime', width:60,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../basicInfo/addExchange.do',//这里是接收数据的URL
						data:$('#exchangeform').serialize(),//传给后台的数据，多个参数用&连接
 ...
 * @param userId
 */
function edit(exchangeId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../basicInfo/getOneExchange.do',//这里是接收数据的URL
		data:'exchangeId='+exchangeId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#exchangeItem').val(userObj.exchangeItem);
			$('#beforeItem').val(userObj.beforeItem);
			$('#afterItem').val(userObj.afterItem);
			$('#exchangeTime').val(userObj.exchangeTime);
