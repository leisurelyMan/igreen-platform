...
jQuery(document).ready(function(){
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../basicInfo/registItemList.do?status=0',//组件创建完成之后请求数据的url
...
function getActions(cellvalue, options, rowObject){
    return '<a href="javascript:edit(\''+rowObject.id+'\')">编辑</a>&nbsp;'
    		+'<a href="javascript:audit (\''+rowObject.id+'\')">审核</a>&nbsp;'
...
function audit(registId){
	if(confirm("确认审核通过？")){
		$.ajax({
			type:'post',//可选get
			async:false,//同步
			url:'../basicInfo/auditRegistItem.do',//这里是接收数据的URL
			data:{"registId":registId},//传给后台的数据，多个参数用&连接
			dataType:'json',//服务器返回的数据类型 可选XML ,Json jsonp script html text等
			success:function(msg){
				if(msg.code == -1){
