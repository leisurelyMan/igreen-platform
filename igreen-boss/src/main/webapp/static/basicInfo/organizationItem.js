...
jQuery(document).ready(function(){
	...
	jQuery("#list1").jqGrid({
		...
		viewrecords : true,
		caption : "注册信息"//表格的标题名字
	});
	
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../basicInfo/organizationItemList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>机构代码</b>','<b>机构名称</b>','<b>机构类型</b>','<b>管辖机构</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'organizationCode',index:'organizationCode', width:100,sortable:false},
					{name:'organizationName',index:'organizationName', width:150,sortable:false},
		 		    {name:'organizationType',index:'organizationType', width:150,sortable:false},
					{name:'jurisdictionalAgency',index:'jurisdictionalAgency', width:60,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../basicInfo/addOrganizationItem.do',//这里是接收数据的URL
						data:$('#organizationform').serialize(),//传给后台的数据，多个参数用&连接
	...
	$("#search").click(function(){
		var searchExchangeItem = $('#searchExchangeItem').val();
		$("#list2").jqGrid("setGridParam",{postData:{exchangeItem:searchExchangeItem},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(organizationItemId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../basicInfo/getOneOrganizationItem.do',//这里是接收数据的URL
		data:'organizationItemId='+organizationItemId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#organizationCode').val(userObj.organizationCode);
			$('#organizationName').val(userObj.organizationName);
			$('#organizationType').val(userObj.organizationType);
			$('#organizationAddress').val(userObj.organizationAddress);
			$('#jurisdictionalAgency').val(userObj.jurisdictionalAgency);
