...
jQuery(document).ready(function(){
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../stopWorld/stopWorldList.do',//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>id</b>','<b>词语</b>','<b>操作</b>' ],//jqGrid的列显示名字
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../stopWorld/addStopword.do',//这里是接收数据的URL
...
function edit(worldId){
	...
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../stopWorld/getOneStopword.do',//这里是接收数据的URL
