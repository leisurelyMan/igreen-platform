...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../relation/branchList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>分支机构名称</b>','<b>分支机构企业注册号</b>','<b>分支机构负责人</b>','<b>一般经营项目</b>','<b>分支机构地址</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'branchName',index:'branchName', width:80,sortable:false},
		 		    {name:'branchRegistNo',index:'branchRegistNo', width:80,sortable:false},
		 		    {name:'principal',index:'principal', width:80,sortable:false},
		 		    {name:'general',index:'general', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../relation/addBranch.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		var searchBranchName = $('#searchBranchName').val();
		var searchBranchRegistNo = $('#searchBranchRegistNo').val();
		var searchPrincipal = $('#searchPrincipal').val();
		
		$("#list2").jqGrid("setGridParam",{postData:{branchName:searchBranchName,branchRegistNo:searchBranchRegistNo,principal:searchPrincipal},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
...
function edit(branchId){
	...
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../relation/getOneBranch.do',//这里是接收数据的URL
		data:'branchId='+branchId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
		    $('#branchName').val(userObj.branchName);
		    $('#branchRegistNo').val(userObj.branchRegistNo);
		    $('#principal').val(userObj.principal);
