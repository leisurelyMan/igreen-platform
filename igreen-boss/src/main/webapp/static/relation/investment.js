...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../relation/investmentList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>企业(机构)名称</b>','<b>注册号</b>','<b>企业(机构)类型</b>','<b>注册资本(万元)</b>','<b>注册资本币种</b>','<b>企业状态</b>','<b>注销日期</b>','<b>吊销日期</b>','<b>登记机关</b>','<b>认缴出资额(万元)</b>','<b>认缴出资币种</b>','<b>出资比例</b>','<b>开业日期</b>','<b>法定代表人姓名</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'companyName',index:'companyName', width:80,sortable:false},
		 		    ...
		 		    {name:'revokeTime',index:'revokeTime', width:80,sortable:false},
		 		    {name:'registOffice',index:'registOffice', width:80,sortable:false},
		 		    {name:'subscribeCapital',index:'subscribeCapital', width:80,sortable:false},
		 		    {name:'subscribeCurrency',index:'subscribeCurrency', width:80,sortable:false},
		 		    {name:'ratio',index:'ratio', width:80,sortable:false},
		 		    {name:'openTime',index:'openTime', width:80,sortable:false},
		 		    {name:'legalRepresentative',index:'legalRepresentative', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../relation/addInvestment.do',//这里是接收数据的URL
	...
	$("#add").click(function(){
		...
		$('#revokeTime').val("");
		$('#registOffice').val("");
		$('#subscribeCapital').val("");
		$('#subscribeCurrency').val("");
		$('#ratio').val("");
		$('#openTime').val("");
 ...
 * @param userId
 */
function edit(investmentId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../relation/getOneInvestment.do',//这里是接收数据的URL
		data:'investmentId='+investmentId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#subscribeCapital').val(userObj.subscribeCapital);
			$('#subscribeCurrency').val(userObj.subscribeCurrency);
