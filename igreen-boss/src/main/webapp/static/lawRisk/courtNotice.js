...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../lawRisk/courtNoticeList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>时间</b>','<b>公告类型</b>','<b>法院</b>','<b>公告内容</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'noticeTime',index:'noticeTime', width:80,sortable:false},
		 		    {name:'noticeType',index:'noticeType', width:80,sortable:false},
		 		    {name:'court',index:'court', width:80,sortable:false},
		 		    {name:'notice',index:'notice', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../lawRisk/addCourtNotice.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		...
		$("#list2").jqGrid("setGridParam",{postData:{noticeType:searchNoticeType,court:searchCourt},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(courtNoticeId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../lawRisk/getOneCourtNotice.do',//这里是接收数据的URL
		data:'courtNoticeId='+courtNoticeId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#noticeTime').val(userObj.noticeTime);
			$('#noticeType').val(userObj.noticeType);
			$('#court').val(userObj.court);
			$('#notice').val(userObj.notice);
