...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../lawRisk/openCourtNoticeList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>标题</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../lawRisk/addOpenCourtNotice.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		...
		$("#list2").jqGrid("setGridParam",{postData:{title:searchTitle},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
...
function edit(courtNoticeId){
	...
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../lawRisk/getOneOpenCourtNotice.do',//这里是接收数据的URL
