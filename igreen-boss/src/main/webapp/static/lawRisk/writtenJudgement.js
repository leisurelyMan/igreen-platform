...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../lawRisk/writtenJudgementList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>时间</b>','<b>案件类型</b>','<b>标题</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'caseTime',index:'caseTime', width:80,sortable:false},
		 		    {name:'caseType',index:'caseType', width:80,sortable:false},
		 		    {name:'caseTitle',index:'caseTitle', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../lawRisk/addWrittenJudgement.do',//这里是接收数据的URL
						data:$('#judgementform').serialize(),//传给后台的数据，多个参数用&连接
	...
	$("#search").click(function(){
		...
		var searchCaseTitle = $('#searchCaseTitle').val();
		$("#list2").jqGrid("setGridParam",{postData:{caseType:searchCaseType,caseTitle:searchCaseTitle},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(judgementId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../lawRisk/getOneWrittenJudgement.do',//这里是接收数据的URL
		data:'judgementId='+judgementId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#caseTime').val(userObj.caseTime);
			$('#caseType').val(userObj.caseType);
			$('#caseTitle').val(userObj.caseTitle);
			//打开对话表
			$("#dialog").dialog("open");
		},
		error:function(){//修理失败，未能连接
