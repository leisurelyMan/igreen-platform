...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../lawRisk/executedItemIdList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>立案时间</b>','<b>案件状态</b>','<b>案号</b>','<b>执行标的</b>','<b>执行法院</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'caseFilingTime',index:'caseFilingTime', width:80,sortable:false},
		 		    {name:'caseStatus',index:'caseStatus', width:80,sortable:false},
		 		    {name:'caseNo',index:'caseNo', width:80,sortable:false},
		 		    {name:'executeTarget',index:'executeTarget', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../lawRisk/addExecutedItem.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		var searchCaseStatus = $('#searchCaseStatus').val();
		var searchCaseNo = $('#searchCaseNo').val();
		var searchExecuteTarget = $('#searchExecuteTarget').val();
		var searchExecuteCourt = $('#searchExecuteCourt').val();
		
		$("#list2").jqGrid("setGridParam",{postData:{caseStatus:searchCaseStatus,caseNo:searchCaseNo,executeTarget:searchExecuteTarget,executeCourt:searchExecuteCourt},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(executedItemId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../lawRisk/getOneExecutedItem.do',//这里是接收数据的URL
		data:'executedItemId='+executedItemId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#caseStatus').val(userObj.caseStatus);
			$('#caseNo').val(userObj.caseNo);
			$('#executeTarget').val(userObj.executeTarget);
