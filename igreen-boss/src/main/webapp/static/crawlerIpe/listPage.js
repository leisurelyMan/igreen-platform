		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../crawlerIpe/addOrEditConfig.do',//这里是接收数据的URL
...
function edit(configId){
	...
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../crawlerIpe/getWebCrawlerConfigById.do',//这里是接收数据的URL
...
function crawler(configId){
    window.open("../crawlerIpe/startCrawler.do?configId=" + configId);
    //取得角色信息
	/*$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../crawlerIpe/startCrawler.do',//这里是接收数据的URL
		data:'configId='+configId,//传给后台的数据，多个参数用&连接
		dataType:'text',//服务器返回的数据类型 可选XML ,Json jsonp script html text等
		success:function(msg){
			/!*if(msg.code == -1){
...
function crawlerTest(configId){
	window.open("../crawlerIpe/startCrawlerTest.do?configId=" + configId);
}
