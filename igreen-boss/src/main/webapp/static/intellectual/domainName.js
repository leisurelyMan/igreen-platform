...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../intellectual/domainNameList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>网址</b>','<b>网址名称</b>','<b>网站备案/许可证号</b>','<b>登记批准日期</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'location',index:'location', width:80,sortable:false},
					{name:'locationName',index:'locationName', width:80,sortable:false},
					{name:'licenseKey',index:'licenseKey', width:80,sortable:false}, 
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../intellectual/addDomainName.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		...
		var searchLocationName = $('#searchLocationName').val();
		$("#list2").jqGrid("setGridParam",{postData:{location:searchLocation,locationName:searchLocationName},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(domainNameId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../intellectual/getOneDomainName.do',//这里是接收数据的URL
		data:'domainNameId='+domainNameId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#locationName').val(userObj.locationName);
			$('#licenseKey').val(userObj.licenseKey);
