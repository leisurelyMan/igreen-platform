...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../intellectual/softwareCopyrightList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>软件名称</b>','<b>软件简称</b>','<b>登记号</b>','<b>版本号</b>','<b>分类号</b>','<b>登记批准日期</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'softwareName',index:'softwareName', width:80,sortable:false},
					{name:'softwareShopName',index:'softwareShopName', width:80,sortable:false},
					{name:'registNo',index:'registNo', width:80,sortable:false}, 
					{name:'softwareVersion',index:'softwareVersion', width:80,sortable:false},
					{name:'classifyNo',index:'classifyNo', width:80,sortable:false}, 
					{name:'approveRegistTime',index:'approveRegistTime', width:80,sortable:false},
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../intellectual/addSoftwareCopyright.do',//这里是接收数据的URL
	...
	$("#add").click(function(){
		...
		$('#id').val("");
		$('#softwareName').val("");
		$('#softwareShopName').val("");
		$('#registNo').val("");
		$('#softwareVersion').val("");
		$('#classifyNo').val("");
		$('#approveRegistTime').val("");
	...
	$("#search").click(function(){
		var searchSoftwareShopName = $('#searchSoftwareShopName').val();
		var searchClassifyNo = $('#searchClassifyNo').val();
		$("#list2").jqGrid("setGridParam",{postData:{softwareShopName:searchSoftwareShopName,classifyNo:searchClassifyNo},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(softwareCopyrightId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../intellectual/getOneSoftwareCopyright.do',//这里是接收数据的URL
		data:'softwareCopyrightId='+softwareCopyrightId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#softwareName').val(userObj.softwareName);
			$('#softwareShopName').val(userObj.softwareShopName);
			$('#registNo').val(userObj.registNo);
			$('#softwareVersion').val(userObj.softwareVersion);
			$('#classifyNo').val(userObj.classifyNo);
			$('#approveRegistTime').val(userObj.approveRegistTime);
