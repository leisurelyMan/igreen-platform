...
jQuery(document).ready(function(){
	...
	//创建jqGrid组件
	jQuery("#list2").jqGrid({
		url : '../intellectual/copyrightList.do?registItemId='+registItemId,//组件创建完成之后请求数据的url
		datatype : "json",//请求数据返回的类型。可选json,xml,txt
		rownumbers: true,
		colNames : ['<b>作品名称</b>','<b>登记号</b>','<b>类别</b>','<b>创作完成日期</b>','<b>登记日期</b>','<b>首次发布日期</b>','<b>最后更新时间</b>','<b>操作</b>' ],//jqGrid的列显示名字
		colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		 		    {name:'productionName',index:'productionName', width:80,sortable:false},
					{name:'registNo',index:'registNo', width:80,sortable:false},
					{name:'productionType',index:'productionType', width:80,sortable:false}, 
					{name:'completeTime',index:'completeTime', width:80,sortable:false},
					{name:'registTime',index:'registTime', width:80,sortable:false}, 
					{name:'firstPublishTime',index:'firstPublishTime', width:80,sortable:false},
					{name:'lastUpdateTime',index:'lastUpdateTime', width:80,sortable:false}, 
		...
		buttons:[//定义两个button按钮
		    {
		    	...
		    	click:function(){
	    			$.ajax({
						type:'post',//可选get
						async:false,//同步
						url:'../intellectual/addCopyright.do',//这里是接收数据的URL
	...
	$("#search").click(function(){
		var searchProductionName = $('#searchProductionName').val();
		var searchProductionType = $('#searchProductionType').val();
		$("#list2").jqGrid("setGridParam",{postData:{productionName:searchProductionName,productionType:searchProductionType},page:1} );//设置查询参数
		$("#list2").trigger("reloadGrid");
	});
});
 ...
 * @param userId
 */
function edit(copyrightId){
	//取得角色信息
	$.ajax({
		type:'post',//可选get
		async:false,//同步
		url:'../intellectual/getOneCopyright.do',//这里是接收数据的URL
		data:'copyrightId='+copyrightId,//传给后台的数据，多个参数用&连接
		...
		success:function(msg){
			...
			$('#id').val(userObj.id);
			$('#productionName').val(userObj.productionName);
			$('#registNo').val(userObj.registNo);
			$('#productionType').val(userObj.productionType);
			$('#completeTime').val(userObj.completeTime);
			$('#registTime').val(userObj.registTime);
			$('#firstPublishTime').val(userObj.firstPublishTime);
			$('#lastUpdateTime').val(userObj.lastUpdateTime);
