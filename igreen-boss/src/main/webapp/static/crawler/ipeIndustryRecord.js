...
var selectedRecord = new Array();
	...
	$("#search").click(function(){
		...
		$("#list").jqGrid("setGridParam",{postData:{webDomain:webDomain,webDetailUrl:webDetailUrl,companyName:companyName,province:province,city:city,punishType:punishType,state:state},page:1} );//设置查询参数
		$("#list").trigger("reloadGrid");
	});
    ...
    $("#yeardelbtn").click(function(){
        ...
        if(webDomain == null || webDomain == ''){
            ...
            $.ajax({
                type:'post',//可选get
                async:false,//同步
                url:'../crawlerIpeIndustry/deleteHistory.do',//这里是接收数据的URL
                data:"webDomain="+webDomain,
                dataType:'json',//服务器返回的数据类型 可选XML ,Json jsonp script html text等
                success:function(msg){
                    if(msg.code == 1){
